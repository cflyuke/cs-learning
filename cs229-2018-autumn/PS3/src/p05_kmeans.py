...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    dist = np.empty((H, W), dtype=int)
    for it in range(max_iter):
        # if it % print_every == 0:
        #     print('Iteration: ', it)
        distances = np.linalg.norm(image[:, :, np.newaxis] - centroids, axis=3)
        dist = np.argmin(distances, axis=2)
        centroids = np.array([np.mean(image[dist == i], axis=0) for i in range(len(centroids)) if len(image[dist == i]) > 0])
