...
def main(is_semi_supervised, trial_num):
    ...
    for i in range(K):
        if i != K-1:
           temp = x[index[i * gap: (i+1) * gap], :]
        else :
           temp = x[index[i*gap: m], :]
        mu_temp = np.mean(temp, axis = 0)
        mu.append(mu_temp)
        sigma_temp = ((temp - mu_temp).T.dot(temp - mu_temp))/ temp.shape[0]
...
def run_semi_supervised_em(x, x_tilde, z, w, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        # Hint: For debugging, recall part (a). We showed that ll should be monotonically increasing.
        # for i in range(K):
        #     w[:,i] = np.exp(-0.5*((x-mu[i]).dot(np.linalg.pinv(sigma[i]))*(x-mu[i])).sum(axis = 1)) / (abs(np.linalg.det(sigma[i]))**0.5) *phi[i]
        # w /= w.sum(axis = 1)[:, None]
        # 初始化变量

         # E-step
        for i in range(K):
            w[:, i] = np.exp(-0.5 * ((x - mu[i]).dot(np.linalg.pinv(sigma[i])) * (x - mu[i])).sum(axis=1)) / (np.linalg.det(sigma[i])**0.5) * phi[i]
        w /= w.sum(axis=1)[:, None]
        ...
        for i in range(K):
            p_xz[:, i] = np.exp(-0.5 * ((x - mu[i]).dot(np.linalg.pinv(sigma[i])) * (x - mu[i])).sum(axis=1)) / (np.linalg.det(sigma[i])**0.5) * phi[i]
        ll = np.sum(np.log(np.sum(p_xz, axis=1) + 1e-10))  # 加上一个小值以避免 log(0)
        # # *** END CODE HERE ***
