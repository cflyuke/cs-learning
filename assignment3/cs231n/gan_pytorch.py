...
def bce_loss(input, target):
    ...
    return bce(input.squeeze(), target.squeeze().type(dtype))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    real_labels = torch.ones_like(scores_real).type(dtype)
    fake_labels = torch.zeros_like(scores_fake).type(dtype)

    real_loss = 0.5*torch.mean((scores_real - real_labels) ** 2)
    fake_loss = 0.5*torch.mean((scores_fake - fake_labels) ** 2)
