...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        xt = x[:, t, :]
        next_h, cachet = rnn_step_forward(xt, prev_h, Wx, Wh, b)
        prev_h = next_h
        h[:, t, :] = next_h
        cache.append(cachet)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        next_h, next_c, cachet = lstm_step_forward(x[:, t, :], prev_h, prev_c, Wx, Wh, b)
        h[:, t, :] = next_h
        prev_h = next_h
        prev_c = next_c
        cache.append(cachet)
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dnext_h += dh[:, t, :]
        cachet = cache[t]
        dx_t, dprev_h_t, dprev_c_t, dWx_t, dWh_t, db_t = lstm_step_backward(dnext_h, dnext_c, cachet)
        dx[:, t, :] = dx_t
        dnext_c = dprev_c_t
        dnext_h = dprev_h_t
        dWx += dWx_t
