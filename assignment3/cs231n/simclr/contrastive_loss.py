...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        pos_sim = sim(z_k, z_k_N) / tau

        neg_sim_sim_k = 0.0
        neg_sim_sim_k_N = 0.0

        for j in range(N):
                neg_sim_sim_k += torch.exp(sim(out_left[j], z_k) / tau)
                neg_sim_sim_k += torch.exp(sim(out_right[j], z_k) / tau)
                neg_sim_sim_k_N += torch.exp(sim(out_left[j], z_k_N) / tau)
                neg_sim_sim_k_N += torch.exp(sim(out_right[j], z_k_N) / tau)
        neg_sim_sim_k -= torch.exp(sim(out_left[k], z_k) / tau)
        neg_sim_sim_k_N -= torch.exp(sim(out_right[k], z_k_N) / tau) 

        total_loss += -torch.log(torch.exp(pos_sim) / neg_sim_sim_k)
        total_loss += -torch.log(torch.exp(pos_sim) / neg_sim_sim_k_N)
...
def compute_sim_matrix(out):
    ...
    normalized_out = out/ torch.linalg.norm(out, dim = 1, keepdims = True)

    sim_matrix = torch.mm(normalized_out, normalized_out.T)
