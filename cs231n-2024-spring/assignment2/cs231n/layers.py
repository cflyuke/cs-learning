...
def affine_forward(x, w, b):
    ...
    out = np.dot(x_reshape, w) + b[np.newaxis, :]
...
def softmax_loss(x, y):
    ...
    num_train = x.shape[0]
    x_scores = np.exp(x)
    x_scores = x_scores / (np.sum(x_scores, axis = 1)[:, np.newaxis])
    loss = np.sum(-np.log(x_scores[np.arange(num_train), y])) /num_train
    x_scores[np.arange(num_train), y] -= 1
    dx = x_scores/num_train
...
def batchnorm_backward(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, D = dout.shape
    x, x_normalized, mean, var, gamma, beta, eps = cache['x'], cache['x_normalized'], cache['mean'], cache['var'], cache['gamma'], cache['beta'], cache['eps']
    ...
    dmean = np.sum(dx_normalized * (-1) / np.sqrt(var +eps), axis = 0) + dvar * np.sum(-2 * (x - mean) / N, axis = 0)
...
def layernorm_backward(dout, cache):
    ...
    dmean = np.sum(dx_normalized * -1 / np.sqrt(var + eps), axis = 1, keepdims = True) + dvar * np.sum(-2 * (x - mean)/ D, axis = 1, keepdims = True)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    N, C, H, W = cache['shape']
    x_group, x_norm, mean, var, gamma, beta, eps, G = cache['x_group'], cache['x_norm'], cache['mean'], cache['var'], cache['gamma'], cache['beta'], cache['eps'], cache['G'] 
    ...
    m = C//G * H * W
    dvar = np.sum(dx_norm * (x_group - mean) * -0.5 * (var + eps)**(-1.5), axis = 1, keepdims = True)
    dmean = np.sum(dx_norm * -1 / np.sqrt(var + eps) , axis = 1, keepdims = True) + dvar * np.sum(-2 * (x_group - mean) / m, axis = 1, keepdims = True)
    dx_group = dx_norm / np.sqrt(var + eps) + dvar * 2 * (x_group - mean) / m + dmean / m
