...
def forward_convolution(conv_W, conv_b, data):
    ...
    num_input_channels, input_width, input_height = data.shape
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    # *** START CODE HERE ***
    grad_bias = np.sum(output_grad, axis=(1,2))
    grad_W = np.zeros(conv_W.shape)
    grad_input_data = np.zeros(data.shape)
    input_channels, input_width, input_height = data.shape
    conv_channels, _, kernel_width, kernel_height = conv_W.shape
    for x in range(input_width - kernel_width + 1):
        for y in range(input_height - kernel_height + 1):
            for out_channel in range(conv_channels):
                grad_W[out_channel, :, :, :] += data[:, x:(x + kernel_width), y:(y + kernel_height)] * output_grad[out_channel, x, y]
                grad_input_data[:, x:(x+kernel_width), y:(y+kernel_height)] += output_grad[out_channel, x, y] * conv_W[out_channel, :, :, :]

    return grad_W, grad_bias, grad_input_data
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    for channel in range(input_channels):
        for x in range(0, input_width, pool_width):
            for y in range(0, input_height, pool_height):
                region = data[channel, x:(x+pool_width), y:(y+pool_height)]
                max_pos = np.unravel_index(np.argmax(region), region.shape)
                grad_data[channel, x + max_pos[0], y + max_pos[1]] = output_grad[channel, x // pool_width, y // pool_height]
    return grad_data
    # *** END CODE HERE ***
...
def backward_linear(weights, bias, data, output_grad):
    ...
    # *** START CODE HERE ***
    return (np.outer(data, output_grad), output_grad, output_grad.dot(weights.T))
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params):
    ...
    back_soft = backward_softmax(logits, grad_cross_entropy)
    back_linear = backward_linear(W2, b2, flattened, back_soft)
    back_relu = backward_relu(first_max_pool, back_linear[2].reshape(first_after_relu.shape))
    back_max_pool = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, back_relu)
    back_convolution = backward_convolution(W1, b1, data, back_max_pool)

    return {
        'W1': back_convolution[0],
        'b1': back_convolution[1],
        'W2': back_linear[0],
        'b2': back_linear[1]
    }
