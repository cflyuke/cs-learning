...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    action_values = mdp_data['value'].dot(mdp_data['transition_probs'][state])
    ...
    for state in range(mdp_data['num_states']):
        ...
        for action in range(2):
            ...
            if total_transitions != 0:
                mdp_data['transition_probs'][state, :, action] = transition_counts[state, :, action] / total_transitions
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        new_value = reward + gamma *(value.dot(transition_probs)).max(axis = 1)
